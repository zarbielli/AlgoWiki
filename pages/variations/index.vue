...
const filteredVariation = computed(() => {
    let result = variations.value
    if (query.value)
        result = variations.value.filter((variation) => {
            return variation.variation.toLowerCase().includes(query.value.toLowerCase())
        })
    return result.sort((a, b) => {
        if (sortBy.value == '# of algorithms') {
            ...
            return b.variation.localeCompare(a.variation)
        }
    })
})
    ...
    <main class="">
        ...
        <div class="max-w-7xl mx-auto mt-8 px-4">
            <ul role="list" class="mt-3 grid grid-cols-1 gap-5 sm:gap-6 sm:grid-cols-2 lg:grid-cols-4">
                <li v-for="(variation, idx) in filteredVariation" :key="idx" class="col-span-1 flex shadow-sm rounded-md">
                    <div
                    ...
                    <div
                        ...
                        <div class="flex-1 px-4 py-2 text-sm truncate">
                            <NuxtLink :to="'/domains/' + variation.domainSlug + '/families/' + variation.familySlug + '/variations/' + variation.variationSlug"
                                class="text-gray-900 font-medium hover:eext-gray-600">{{ variation.variation }}</NuxtLink>
